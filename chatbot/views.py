...
def get_gemini_response(user_input):
    ...
    if not api_key:
        raise ValueError("API key not found. Make sure to set GEMINI_API_KEY in your environment variables.")
    ...
    if response.status_code == 200:
        return response.json()["candidates"][0]["output"]  # Extract the generated text
    else:
...
def negotiate(request):
    if request.method == 'POST':
        ...
        # Determine if we should use Gemini or negotiation logic
